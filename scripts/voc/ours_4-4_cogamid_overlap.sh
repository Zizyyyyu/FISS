...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} fl_main.py --date ${START_DATE} --data_root ${DATA_ROOT} --overlap --dataset ${DATASET} --name ${NAME} --task ${TASK} --incremental_method ${INCREMENTAL_METHOD} --class_ratio ${CLASS_RATIO} --sample_ratio2 ${SAMPLE_RATIO2} --batch_size ${BATCH_SIZE} --epochs_local ${EPOCHS_LOCAL} --steps_global ${STEPS_GLOBAL} --epochs_global ${EPOCHS_GLOBAL} --seed ${SEED} --gmm_components ${GMM_COMPONENTS} --gmm_min_features ${GMM_MIN_FEATURES} --gmm_max_features ${GMM_MAX_FEATURES} --gmm_pseudo_threshold ${GMM_PSEUDO_THRESHOLD} --gmm_em_iters ${GMM_EM_ITERS} --cogamid_mbce ${COGAMID_MBCE} --cogamid_pkd ${COGAMID_PKD} --cogamid_cont ${COGAMID_CONT} --cogamid_uncer ${COGAMID_UNCER} --cogamid_pos_weight ${COGAMID_POS_WEIGHT} --cogamid_replay_max_per_class ${COGAMID_REPLAY_MAX_PER_CLASS} --cogamid_feature_noise ${COGAMID_FEATURE_NOISE} --opt_level O1
